...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden bg-white shadow-lg dark:bg-gray-900">
      {/* Image */}
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        setLoading(true);
        setError(null);
        const response = await apiClient.get(getApiEndpoint.getproduct("lumora-magazine"));
